...
3. **End-to-End Post-Quantum Cryptography (PQC):** Replaces legacy asymmetric primitives (RSA, ECDSA, ECDH) with NIST FIPS-standardized post-quantum algorithms: **ML-KEM-768 (FIPS 203)** for key encapsulation, **ML-DSA-65 (FIPS 204)** for digital signatures and consensus votes, and **SLH-DSA (FIPS 205)** for immutable root anchors.
...
| **NIST-standardized Post-Quantum Cryptography** | Resistance against Store-Now-Decrypt-Later (SNDL) and future quantum forgery. | Classical RSA/ECC or hybrid crypto masquerading as quantum-safe. | **ML-KEM-768 (FIPS 203)** for key wrapping; **ML-DSA-65 (FIPS 204)** for requests & blocks; **SLH-DSA (FIPS 205)** for roots. |
...
| **Long-Lived Root Auth** | SLH-DSA-128s | NIST FIPS 205 | PK: 32 B<br>Sig: 7,856 B | Stateless Hash-Based Signature. Ultra-conservative security for validator genesis & checkpoints. |
